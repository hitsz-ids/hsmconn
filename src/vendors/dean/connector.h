...
class Connector : public hsmc::Connector {
 public:
  ...
  int SDF_GetSesKey(void *hSessionHandle, void *phKeyHandle);

 private:
  /// HSM设备句柄
  sdf_handle_t hDevice_;

  using SDF_OpenDeviceWithPath_t = int (*)(char *pConfPath, void **phDeviceHandle);
  SDF_OpenDeviceWithPath_t SDF_OpenDeviceWithPath_;
  using SDF_GetSesKey_t = int (*)(void *hSessionHandle, void *phKeyHandle);
  SDF_GetSesKey_t SDF_GetSesKey_;
};
