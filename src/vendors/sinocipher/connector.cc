...
void Connector::open() {
  ...
  SDF_OpenDeviceWithConfigPath_ = resolveFunc<SDF_OpenDeviceWithConfigPath_t>("SDF_OpenDeviceWithConfigPath");
  rc = SDF_OpenDeviceWithConfigPath_(&hDevice_, (unsigned char *)getConfig().c_str());
  if (SDR_OK != rc) {
    std::string errorMsg = absl::StrFormat("[%s] fail to execute SDF_OpenDeviceWithConfigPath", getName().c_str());
    throw SdfExcuteException(errorMsg, rc);
  }
}
