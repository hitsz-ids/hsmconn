...
bool SessionImpl::isGood(int *errcode, bool *dev_reopen) const {
  ...
  if (nullptr == connect) {
    throw SdfExcuteException(
        absl::StrFormat("Fail to dynamic_cast from hsmc::Connector to hsmc::svs::ndsec::Connector"));
  }
  return 0 == connect->SVS_GenerateRandom(hSession_, 6, randomData);
}
...
int SessionImpl::SVS_GenerateRandom(int length, uint8_t *randomData) const {
  auto connect = dynamic_cast<Connector *>(connector_.get());
  if (nullptr == connect) {
    throw SdfExcuteException(
        absl::StrFormat("Fail to dynamic_cast from hsmc::Connector to hsmc::svs::ndsec::Connector"));
  }
  return connect->SVS_GenerateRandom(getSessionHandle(), length, randomData);
}
