...
void Connector::open() {
  ...
  SDF_OpenDeviceWithPathAndName_ = resolveFunc<SDF_OpenDeviceWithPathAndName_t>("SDF_OpenDeviceWithPathAndName");
  rc = SDF_OpenDeviceWithPathAndName_((unsigned char *)getConfig().c_str(), &hDevice_);
  if (SDR_OK != rc) {
    std::string errorMsg = absl::StrFormat("[%s] fail to execute SDF_OpenDeviceWithPathAndName", getName().c_str());
    throw SdfExcuteException(errorMsg, rc);
  }
}
